...
void main(void) {
    ...
    for (int frame = 0; frame < 8; frame++) {
        ...
        if (frame == 5) {
            ...
            /* X68Cmd縮小(2026-09-01)の検証用: クリップ前の生の座標(p0..p3)が
             * 画面外にある命令。rect側はクリップ後の値(short化)しか持たないが、
             * p0..p3は生のまま(intのまま)なので、記録・再生とも正しく動く
             * はず(docs/L1差分描画_20260901.md参照)。 */
            x68_box_fill(-20, 400, 60, 40, c_edge); /* 左端が画面外: 部分的に見えるはず */
            x68_line(-30, 450, 60, 470, c_line);    /* 片方の端点が画面外: 部分的に見えるはず */
        }

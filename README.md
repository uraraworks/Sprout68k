...
その後、WorkbenchNP2を土台にSprout68kのIDEを9段で実装した。現在は、CodeMirrorでの
...
[`docs/コンパイル診断注釈_20260822.md`](docs/コンパイル診断注釈_20260822.md)、
[`docs/IDE第5-9段と公開前整備_20260822.md`](docs/IDE第5-9段と公開前整備_20260822.md)を参照。

第5段以降では、本家cc1 / ldから採取した実診断に対し、原文を残したまま
「何が起きたか／次にすること」を重ねる日本語注釈層を実装した。実行のたびに
`runFresh()`でエミュレータを作り直す1クリック復帰は、編集中のソースを変更しない。
キーボード入力は9種類がゲストへ届くことを実測し、ホスト側に対応キーがないX68000の
キー10種類は未対応として棚卸しした。

Service Workerによる配布経路も実装し、Chromium系ブラウザ1種でサーバー停止後に
書く→ビルド→実行まで通ることを実測した。約20MBのwasmツールをGitHub Releaseから
取得・検証する経路は用意したが、リモートとReleaseはまだ作成していない。公開前の
受け取りポリシー、日英1ファイルのヘルプ、ふたばのアイコンを加え、製品名は
Sprout68kに確定した。

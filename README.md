...
故障注入も不一致として検出した。**ここまでで完了したのはブラウザ内ビルドとバイト一致の
確認であり、製品としてのブラウザ結線全体ではない。** 配布・Cache Storage・オフライン
設計、1クリック復帰とソース保持を備えた学習者向けUI、コンパイルエラーの日本語注釈、
生成 `.xdf` をブラウザ内px68kで起動する結線、Safari / Firefox実測は未完了である。
詳細: [`docs/コンパイラwasm化_20260820.md`](docs/コンパイラwasm化_20260820.md)、
[`docs/ブラウザ結線_20260821.md`](docs/ブラウザ結線_20260821.md)。

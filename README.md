...
ホスト側の wasm クロスビルドは着手済み。driver を外した直呼び経路（F-1）と、
binutils 2.44 + GCC 13.4.0 のネイティブ基準器（F-0）、ツール単位で native / wasm を
差し替える駆動層、binutils の wasm 化（F-2）まで作成・検証済み。F-2 は Stage C /
breakout ともツール単位の混成5構成で全成果物が正典ネイティブ版とバイト一致した。
**`cc1` の wasm 化（F-3）とブラウザ結線（F-4）は未完了。** 詳細:

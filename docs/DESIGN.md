# Sprout68k 設計・実装・実測記録

利用者向けの説明は [README.md](../README.md) / [README.ja.md](../README.ja.md) を参照。
このファイルは開発の実測記録であり、時系列で古い内容を含む。既定値等が後の節で
変わっている場合はその旨を明記してある。

## docs/ 資料索引

- [`実機互換_要件追加_20260819.md`](実機互換_要件追加_20260819.md) — 実機(当時のX68000)
  で動くことを要件に追加し、スタック位置と命令キャッシュの非互換2点を修正した記録
- [`toolchain調査.md`](toolchain調査.md) — elf2x68k/xdev68k のビルド定義から確定した
  gcc/binutilsのバージョンとconfigureオプションの調査結果
- [`API設計_20260819.md`](API設計_20260819.md) — 学習用APIの第一版設計とStage E実測項目
- [`StageE-1_実測_20260819.md`](StageE-1_実測_20260819.md) — 65536色1ページのGVRAM論理
  サイズとアドレッシングの実測
- [`StageE-2-3_実測_20260819.md`](StageE-2-3_実測_20260819.md) — 垂直同期の検出方法と
  裏バッファ→表バッファの実測
- [`StageE-4_実測_20260819.md`](StageE-4_実測_20260819.md) — `key_down(key)` 実装のための実測
- [`StageE-5_実測_20260819.md`](StageE-5_実測_20260819.md) — 例外を捕捉して「見える形で
  止まる」実測
- [`StageE-6_実測_20260819.md`](StageE-6_実測_20260819.md) — `print_at(col, row, s)` 実装
  のための実測
- [`lib実装_20260819.md`](lib実装_20260819.md) — 学習用ライブラリ第一版(L0 + 標準名の層)
  の実装記録
- [`L1実装_20260819.md`](L1実装_20260819.md) — 学習用ライブラリL1(画面)の実装記録
- [`作例breakout_20260819.md`](作例breakout_20260819.md) — 作例「ブロック崩し」実装記録
- [`パニック画面_20260820.md`](パニック画面_20260820.md) — パニック画面の実装と68030
  VBR対応
- [`重なり実測_20260820.md`](重なり実測_20260820.md) — テキスト面とグラフィック面の
  重なり方の実測
- [`VC重畳実測_20260820.md`](VC重畳実測_20260820.md) — 65536色グラフィックとテキストの
  同時表示可否の訂正実測
- [`コンパイラwasm化_20260820.md`](コンパイラwasm化_20260820.md) — コンパイラをwasm化した
  作業記録(F-4)
- [`ブラウザ結線_20260821.md`](ブラウザ結線_20260821.md) — ブラウザ結線F-4の実測記録
- [`IDE実装_20260822.md`](IDE実装_20260822.md) — IDEシェル〜px68k実行までの実装・実測記録
  (第1〜4段)
- [`コンパイル診断注釈_20260822.md`](コンパイル診断注釈_20260822.md) — コンパイル診断への
  日本語注釈の実測記録
- [`IDEキーボード入力_20260822.md`](IDEキーボード入力_20260822.md) — IDEのキーボード入力
  実測記録
- [`IDE配布設計_20260822.md`](IDE配布設計_20260822.md) — Service Workerによるオフライン
  配布設計
- [`IDE第5-9段と公開前整備_20260822.md`](IDE第5-9段と公開前整備_20260822.md) — IDE第5〜9段
  (診断注釈・1クリック復帰・キーボード・オフライン・Release配布経路)と公開前整備の記録
- [`Release配布手順_20260822.md`](Release配布手順_20260822.md) — wasmツールのGitHub
  Release／GitHub Pages配布手順
- [`共有ランタイム配布_20260822.md`](共有ランタイム配布_20260822.md) — 共有リンク`#p1=`を
  開く側が持つランタイムの配り方
...
[`実機互換_要件追加_20260819.md`](実機互換_要件追加_20260819.md)を参照。

現在の既定値は `STACK_ADDR=$F0000`(1MB機向け)。`$1F0000`(2MB機向け)は大サイズ試験
(約1MB本体・ディスク全体)でのみ明示的に使う。以下の Stage D の節に出てくる
「既定 `$1F0000`」という記述は、この実機互換対応より前の当時の既定値の記録である。
...
このStage D対応を行った当時、スタックを `STACK_ADDR`(ビルドスクリプトが `-D` で
埋め込む。**当時の既定** `$1F0000`)へ変更した(`stage_c/crt0/crt0.S`、
`stage_c/boot/boot.S`、`stage_d/boot/boot.S`)。`$1F0000` は検証ハーネスが px68k
に設定する `px68k_ramsize=2MB`(=`$200000`)の範囲内に収まり、ディスク全体
(1231セクタ=1,260,544バイト、本体が取り得る最大)を読み込んでも本体末尾
(`$3000+1,260,544=$137200`)と衝突しない。
...
この後の「実機互換の要件追加(2026-08-19)」で既定値は `STACK_ADDR=$F0000` に
変更され、`$1F0000` は大サイズ試験でのみ明示指定する値になった。

### 修正後の再測定(px68k_ramsize=2MB、`verify/verify.mts` Stage D、当時の既定 `STACK_ADDR=$1F0000`)
...
[`API設計_20260819.md`](API設計_20260819.md) を参照。
**Stage E は全項目(E-1〜E-6)の実測が完了した。** 各実測資料:
[`StageE-1_実測_20260819.md`](StageE-1_実測_20260819.md)、
[`StageE-2-3_実測_20260819.md`](StageE-2-3_実測_20260819.md)、
[`StageE-4_実測_20260819.md`](StageE-4_実測_20260819.md)、
[`StageE-5_実測_20260819.md`](StageE-5_実測_20260819.md)、
[`StageE-6_実測_20260819.md`](StageE-6_実測_20260819.md)。

学習用ライブラリ（L0 / 標準名の層 / L1）と作例ブロック崩しは実装・実測済み。
詳細: [`lib実装_20260819.md`](lib実装_20260819.md)、
[`L1実装_20260819.md`](L1実装_20260819.md)、
[`作例breakout_20260819.md`](作例breakout_20260819.md)、
[`パニック画面_20260820.md`](パニック画面_20260820.md)、
[`重なり実測_20260820.md`](重なり実測_20260820.md)、
[`VC重畳実測_20260820.md`](VC重畳実測_20260820.md)。
...
[`コンパイラwasm化_20260820.md`](コンパイラwasm化_20260820.md)と
[`ブラウザ結線_20260821.md`](ブラウザ結線_20260821.md)に記録している。
...
[`IDE実装_20260822.md`](IDE実装_20260822.md)と
[`コンパイル診断注釈_20260822.md`](コンパイル診断注釈_20260822.md)、
[`IDE第5-9段と公開前整備_20260822.md`](IDE第5-9段と公開前整備_20260822.md)を参照。
...
取得・検証する経路も用意し、その後 `runtime-v1`(共有ランタイム)・`wasm-tools-v1`
の各Releaseと GitHub Pages 公開を実際に行った。公開前の
...
- 68kソース行デバッグ
- Safari / Firefoxでは未実測で、ブラウザ互換性を確認済みとはしていない

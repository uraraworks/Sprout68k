#!/usr/bin/env bash
# Sprout68k: samples/breakout/block.c(素のコード)と検証用の版(HOSTVAR書き出し+
# 故障注入マクロ入り)との差分を verify/patches/breakout_verify.patch として
# 保存し直す。
#
# 通常は使わない。samples/breakout/block.c を編集した結果、
...
#
# 使い方:
#   1. samples/breakout/block.c をエディタで直接書き換え、
...
ROOT="$(cd "$HERE/.." && pwd)"
CLEAN="$ROOT/samples/breakout/block.c"

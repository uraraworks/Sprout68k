#!/usr/bin/env bash
# Stage E-6(文字表示の座標指定 print_at の実測)用ビルド。crt0/IOCS $21 スタブ・
# リンカスクリプト・ブートセクタは Stage C のものをそのまま流用する(本体は
# 7セクタ以内に収まる小さいプログラムのため、track/side またぎに対応した
# Stage D のブートセクタは不要)。IOCS $23(候補、カーソル位置指定)のスタブは
# stage_e/src/iocs_e6.S に新規追加。
#
# 使い方: tools/build_stage_e6.sh <marker_spec_json> <output.xdf>
#   marker_spec_json: '[]' (マーカー無し) か '[[col,row,use_locate,"text"],...]'
#   例: tools/build_stage_e6.sh '[[2,1,1,"HELLO"]]' build/stage_e6_test.xdf
#
...
ROOT="$(cd "$HERE/.." && pwd)"
OBJDIR="$ROOT/build/stage_e6_obj"
mkdir -p "$OBJDIR"
...
python3 "$ROOT/tools/gen_markers_e6.py" "$MARKER_SPEC" "$OBJDIR/markers_data.c"

echo "== Stage E-6 本体(C)のビルド =="
m68k-elf-gcc "${CFLAGS[@]}" -c "$ROOT/stage_e/src/main_e6.c" -o "$OBJDIR/main.o"
...
m68k-elf-ld -T "$ROOT/stage_c/crt0/linker.ld" -o "$OBJDIR/stage_e6.elf" \
  "$OBJDIR/crt0.o" "$OBJDIR/iocs.o" "$OBJDIR/iocs_e6.o" "$OBJDIR/main.o" "$OBJDIR/markers_data.o"
m68k-elf-objcopy -O binary "$OBJDIR/stage_e6.elf" "$OBJDIR/stage_e6.bin"

BODY_SIZE=$(wc -c < "$OBJDIR/stage_e6.bin" | tr -d ' ')
...
echo "== .xdf の合成 =="
python3 - "$OBJDIR/boot.bin" "$OBJDIR/stage_e6.bin" "$SECTOR_COUNT" "$OUT_XDF" <<'PYEOF'
import sys
